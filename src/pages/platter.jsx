import React, { useState, useEffect } from "react";
import { ArrowLeft } from "lucide-react";
import { useNavigate, useParams } from "react-router-dom";
import PlatterCard from "../components/platter/card";

const Platter = () => {
  const navigate = useNavigate();
  const { platterType } = useParams();
  const [platterData, setPlatterData] = useState(null);

  const handleGoBack = () => {
    navigate(-1);
  };

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/data.json");
        const data = await response.json();
        setPlatterData(data[platterType]); // Access data based on platterType
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    };

    fetchData();
  }, [platterType]);

  if (!platterData) {
    return <div>Loading...</div>;
  }

  return (
    <div className="bg-[#FFF0D8] min-h-screen flex flex-col items-center px-4 sm:px-6 md:px-10 py-6 gap-3">
      {/* Logo */}
      <div>
        <img
          className="w-auto h-16 sm:h-20"
          src="/images/logo-black.png"
          alt="Restaurant Logo"
        />
      </div>

      {/* Back Button and Heading */}
      <div className="relative w-full flex items-start">
        <div
          className="hidden absolute md:flex items-center space-x-2 hover:cursor-pointer"
          onClick={handleGoBack}
        >
          <ArrowLeft />
          <p className="text-lg sm:text-xl">Back</p>
        </div>
        <h2 className="ml-auto mr-auto text-xl sm:text-2xl md:text-3xl uppercase font-semibold">
          {platterData.heading}
        </h2>
      </div>

      {/* Description */}
      <p className="max-w-xl text-center text-sm sm:text-base text-black/40">
        {platterData.description}
      </p>

      {/* Platter Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
        {platterData.dishes.map((dish) => (
          <PlatterCard key={dish.id} dish={dish} />
        ))}
      </div>
    </div>
  );
};

export default Platter;
